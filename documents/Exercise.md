...
Приложение расположено в файле [`aqa-shop.jar`](https://github.com/netology-code/aqa-qamid-diplom/blob/main/aqa-shop.jar) и запускается стандартным способом `java -jar aqa-shop.jar` на порту 8080.

В файле [`application.properties`](https://github.com/netology-code/aqa-qamid-diplom/blob/main/application.properties) дан ряд типовых настроек:
...
Учётные данные и URL для подключения задаются в файле [`application.properties`](https://github.com/netology-code/aqa-qamid-diplom/blob/main/application.properties).

### Банковские сервисы

Симулятор банковских сервисов запущен по адресу `185.119.57.197:9999`. Этот адрес уже указан в файле [`application.properties`](https://github.com/netology-code/aqa-qamid-diplom/blob/main/application.properties).
